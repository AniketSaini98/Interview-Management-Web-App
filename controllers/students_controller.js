    ...
    if (!!(DSA && WEB && REACT)) {
      ...
      for (let COURSE of COURSES) {
        ...
        if (!enrolment) {
          ...
          BATCH.enrollments.push(enrolment);
          await BATCH.save();
          //Add the enrolment to the course
          COURSE.enrollments.push(enrolment);
          await COURSE.save();
          ...
          await Batch.findOneAndUpdate(
            { "enrollments._id": enrolment._id },
            {
              $set: {
                "enrollments.$.batch": enrolment.batch,
                "enrollments.$.course": enrolment.course,
              },
            }
          );
...
module.exports.delete = async (req, res) => {
  try {
    ...
    if (batch.students.length === 0) {
      let enrollments = await Enrolment.find({ batch: batch._id });
      for (let enrolment of enrollments) {
        //Updating the Course by deleting the enrolment
        let course = await Course.findById(enrolment.course._id);
        course.enrollments.pull(enrolment);
        await course.save();
        //Delete Enrolment
    ...
    if (students.length === 0) {
      ...
      await Company.updateMany({}, { $set: { interviews: [], results: [] } });
    }
...
//4. Update Enrollments List for the updated Batch and Course
//5. Update those Courses & Batches which have the above Enrollments List
//6. Delete Batch if it has no Students
//-	Updating those Courses which have the above Enrollments List
//-	Delete Enrolment
